...
Framebuffer *initialize_gop(EFI_SYSTEM_TABLE *SystemTable) {
  ...
  SystemTable->BootServices->AllocatePool(EfiLoaderData,
                                          ...
                                          (void **)&frame_buffer.back_buffer);
  SystemTable->BootServices->SetMem(frame_buffer.back_buffer,
                                    frame_buffer.buffer_size, 0x00);
...
EFI_FILE *LoadFile(EFI_FILE *Directory, CHAR16 *Path, EFI_HANDLE ImageHandle,
  ...
  SystemTable->BootServices->HandleProtocol(loaded_image->DeviceHandle,
                                            &gEfiSimpleFileSystemProtocolGuid,
                                            (void **)&file_system);

  if (Directory == NULL) {
    file_system->OpenVolume(file_system,
                            &Directory); // Set to root of file system if null
  }

  EFI_STATUS s = Directory->Open(Directory, &loaded_file, Path,
...
EFI_STATUS efi_main(EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE *SystemTable) {
  ...
  Framebuffer *new_buffer =
      initialize_gop(SystemTable); // graphics output protocol

  void (*KernelStart)(Framebuffer *, PSF1_FONT *, EFI_MEMORY_DESCRIPTOR *,
                      UINTN, UINTN, UINT32) =
      ((__attribute__((sysv_abi)) void (*)(Framebuffer *, PSF1_FONT *,
                                           EFI_MEMORY_DESCRIPTOR *, UINTN,
                                           UINTN, UINT32))header.e_entry);

  PSF1_FONT *newfont =
      load_psf1_font(NULL, L"zap-light16.psf", ImageHandle, SystemTable);
